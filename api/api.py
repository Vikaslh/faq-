...
from .faq_generator import generate_faqs
# api.py
from .utils import extract_text_from_pdf  # Import the helper function

api = NinjaAPI()

"""
@api.post("/documents/")
def add_document(request, title: str, description: str, content: str, uploaded_by: str):
   # document = Document.objects.create(title=title, description=description, content=content, uploaded_by=uploaded_by)
    document=File.read()
...
@api.get("/documents/{document_id}/faqs/")
...
def update_document(request, document_id: int, title: str = None, description: str = None, content: str = None):
    document = get_object_or_404(Document, id=document_id)
    if title: document.title = title
    if description: document.description = description
    if content:
        document.content = content
        document.faqs.all().delete()  # Regenerate FAQs
        faqs = generate_faqs(content)
        for faq in faqs:
            FAQ.objects.create(document=document, question=faq['question'], answer=faq['answer'])
    document.save()
...
"""
    try:
        ...
        return {"success": True, "document_id": document.id}, 201  # HTTP 201 Created
